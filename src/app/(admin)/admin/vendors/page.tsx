  ...
  return (
    <div className="space-y-10">
      <header className="flex flex-col md:flex-row justify-between md:items-end gap-4">
        <div>
          <h1 className="text-4xl font-black tracking-tight text-slate-900 mb-2">Vendor Directory</h1>
          <p className="text-slate-500 font-medium text-lg">Managing {vendors.length} storefronts across the platform.</p>
        </div>
      ...
      <section className="bg-white rounded-3xl border border-slate-200 overflow-hidden shadow-sm">
          ...
          <table className="w-full text-left">
            <thead>
              <tr className="bg-slate-50/50 border-b border-slate-100">
                <th className="px-8 py-5 text-[10px] font-black uppercase tracking-widest text-slate-400">Storefront</th>
                ...
                <th className="px-6 py-5 text-[10px] font-black uppercase tracking-widest text-slate-400">Sales</th>
                ...
                >
                  <td className="px-8 py-5">
                    <div className="flex items-center gap-4">
                      <div className="w-12 h-12 rounded-xl overflow-hidden relative bg-slate-100 border border-slate-200">
                        {vendor.image ? (
                          <Image src={vendor.image} alt={vendor.name} fill className="object-cover" sizes="48px" />
                        ) : (
                      ...
                      <div>
                        <div className="flex items-center gap-1.5">
                           ...
                           {vendor.isVerified && <CheckCircle2 className="w-3.5 h-3.5 text-blue-500" />}
                        </div>
                        <p className="text-xs text-slate-400 font-medium tracking-tight">/{vendor.slug}</p>
                      </div>
                    </div>
                  </td>
                  ...
                  <td className="px-6 py-5">
                    <div className="flex items-center gap-1">
                      ...
                      <span className="text-sm font-bold text-slate-700">{vendor.rating.toFixed(1)}</span>
                    </div>
                  </td>
                  <td className="px-6 py-5">
                  ...
                  <td className="px-6 py-5">
                    <p className="text-sm font-black text-slate-900">{vendor.totalSales} orders</p>
                  </td>
        ...
        {loading && (
          <div className="p-12 text-center">
             ...
             <p className="text-slate-400 font-bold text-sm">Synchronizing Vendor Database...</p>
          </div>
        )}
      </section>
    </div>
