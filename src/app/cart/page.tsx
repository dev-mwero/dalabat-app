    ...
    <main className="max-w-screen-2xl mx-auto px-6 py-12 mb-24 md:mb-0">
      ...
      {items.length === 0 ? (
         <div className="text-center py-20 bg-surface-container-low rounded-2xl">
            ...
            <h2 className="text-2xl font-bold text-on-surface mb-2">Your basket is empty</h2>
            <p className="text-on-surface-variant mb-8">Looks like you haven't added any artisanal provisions yet.</p>
            <Link href="/" className="bg-primary-container text-white px-8 py-3 rounded-full font-bold hover:opacity-90 transition-opacity">
                Start Exploring
            </Link>
         </div>
      ) : (
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
        ...
        <div className="lg:col-span-8 space-y-6">
              ...
              >
                ...
                <div className="flex-grow flex flex-col md:flex-row justify-between w-full">
                  <div className="space-y-1 text-center md:text-left">
                    ...
                    <h3 className="text-xl font-bold text-on-surface">{item.product.name}</h3>
                    <p className="text-sm text-on-surface-variant line-clamp-1">{item.product.description}</p>
                  </div>
          ...
          <div className="mt-12 p-8 rounded-xl bg-surface-container-low flex flex-col md:flex-row items-center gap-8 overflow-hidden relative group">
            <div className="relative z-10 space-y-4 md:w-2/3">
              <h4 className="text-2xl font-extrabold text-on-surface tracking-tight">Complete your pantry</h4>
              <p className="text-on-surface-variant text-sm md:text-base leading-relaxed">
                Add our <span className="font-bold text-primary">Smoked Sea Salt</span>—harvested by hand in small batches. It's the perfect pairing for your artisanal selection.
              </p>
              <button 
        ...
        <aside className="lg:col-span-4 sticky top-28">
          <div className="bg-surface-container-highest/30 backdrop-blur-md rounded-xl p-8 space-y-8 border border-outline-variant/10 shadow-lg">
            <h2 className="text-2xl font-bold tracking-tight text-on-surface">Order Summary</h2>
            ...
            <div className="space-y-4">
              ...
              <div className="flex justify-between items-center text-on-surface-variant">
                <span className="text-sm font-semibold">Estimated Shipping</span>
                <span className="font-bold text-on-surface">{totalPrice > 50 ? "Free" : "$5.99"}</span>
              </div>
              <div className="flex justify-between items-center text-on-surface-variant">
                <span className="text-sm font-semibold">Estimated Tax</span>
                <span className="font-bold text-on-surface">${(totalPrice * 0.08).toFixed(2)}</span>
              </div>
            ...
            <div className="pt-4 flex flex-col items-center gap-3">
              ...
              <p className="text-[10px] text-on-surface-variant text-center leading-tight">
                Your order is protected by our Curator Guarantee. <br/>Secure transaction processed via Zest Pay.
              </p>
            </div>
          </div>
          ...
          <div className="mt-6 flex items-center justify-center gap-2 text-on-surface-variant">
             <Link href="/support" className="text-sm font-semibold hover:text-primary underline">
